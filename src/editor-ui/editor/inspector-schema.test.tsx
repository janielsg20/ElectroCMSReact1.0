...
describe('M07.1 inspector declarativo', () => {
  ...
  it('presenta descriptor, valor efectivo y control tipado conectado a la sesión', async () => {
    ...
    expect(screen.getByText('maxWidth · Número')).toBeInTheDocument()
    ...
    const input = screen.getByRole('spinbutton', { name: 'Ancho máximo maxWidth · Número' })
  ...
  it('muestra el error de validación junto al campo y lo anuncia', async () => {
    ...
    fireEvent.change(screen.getByRole('spinbutton', { name: 'Ancho máximo maxWidth · Número' }), { target: { value: '-1' } })
    fireEvent.click(screen.getByRole('button', { name: 'Aplicar' }))
