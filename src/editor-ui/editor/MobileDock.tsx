          ...
          return (
            <li className="min-w-0" key={item.label}>
              <button
                ...
                className={`mobile-dock-option relative flex h-full w-full cursor-pointer flex-col items-center justify-center gap-0.5 px-0.5 text-[0.625rem] font-medium transition-colors ${active ? 'mobile-dock-option--active text-primary-strong' : 'text-muted-foreground'} ${disabled ? 'opacity-40' : ''}`}
                disabled={disabled}
                onClick={() => item.id !== 'more' && onPanelChange(selected ? null : item.id)}
                title={disabled ? 'Más herramientas · planificado' : item.label}
                type="button"
              >
