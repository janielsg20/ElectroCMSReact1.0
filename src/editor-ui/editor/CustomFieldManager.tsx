...
export function CustomFieldManager() {
  ...
  const contentTypes = useMemo(
    () => Object.values(structure.cms?.contentTypes ?? {}).sort((a, b) => a.pluralName.localeCompare(b.pluralName, 'es')),
    [structure.cms?.contentTypes],
  )
  const taxonomies = useMemo(
    () => Object.values(structure.cms?.taxonomies ?? {}).sort((a, b) => a.pluralName.localeCompare(b.pluralName, 'es')),
    [structure.cms?.taxonomies],
  )
  ...
  function patchDraft(patch: Partial<FieldDraft>): void {
  ...
  function setOwnerKind(ownerKind: OwnerKind): void {
    patchDraft({
      ownerKind,
      ownerId: ownerKind === 'content-type' ? contentTypes[0]?.id ?? '' : taxonomies[0]?.id ?? '',
      childFieldIds: [],
      relationId: '',
      taxonomyId: '',
      conditionsJson: '[]',
  ...
  function toggleStringList(key: 'allowedRoleIds' | 'childFieldIds', value: string): void {
    patchDraft({
      [key]: draft[key].includes(value) ? draft[key].filter((id) => id !== value) : [...draft[key], value],
    })
  }

  function buildCandidate(): ReturnType<typeof FieldDefinitionSchema.safeParse> {
    let conditions: unknown
    try { conditions = JSON.parse(draft.conditionsJson || '[]') } catch {
      return FieldDefinitionSchema.safeParse({ invalid: 'conditions-json' })
    }
    let options: unknown[]
    try { options = parseOptions(draft.optionsText) } catch {
      return FieldDefinitionSchema.safeParse({ invalid: 'options-text' })
    }

    const owner = draft.ownerKind === 'content-type'
      ? contentTypes.find((item) => item.id === draft.ownerId)
      : taxonomies.find((item) => item.id === draft.ownerId)
    ...
    return FieldDefinitionSchema.safeParse({
      ...
      owner: draft.ownerKind === 'content-type'
        ? { contentTypeId: owner.id, kind: 'content-type' }
        : { kind: 'taxonomy', taxonomyId: owner.id },
      ...
      type: draft.type,
      validation: {
        max: parseNullableNumber(draft.max),
        maxLength: parseNullableInteger(draft.maxLength),
        min: parseNullableNumber(draft.min),
        minLength: parseNullableInteger(draft.minLength),
  ...
  async function saveField(): Promise<void> {
    ...
    if (!parsed.success) {
      setMessage(parsed.error.issues[0]?.message ?? 'Revisa la configuración del campo.')
      return
    }
    setPending(true)
    if (selected) {
      ...
      if (created.ok) {
        ...
        setMessage(`${parsed.data.label} creado y vinculado a su propietario.`)
      } else {
  ...
  async function removeField(): Promise<void> {
    ...
    if (deleteArmedId !== selected.id) {
      ...
      setMessage('Pulsa Confirmar eliminación. Los campos usados por datos, composición, condiciones, queries, formularios o roles quedan protegidos.')
      return
    }
    setPending(true)
  ...
  return (
    <section aria-labelledby="custom-fields-title" className="grid gap-2 p-2 lg:p-1.5">
      <div className="flex items-start justify-between gap-2 rounded-md border border-border bg-muted/30 p-2">
        <div className="flex min-w-0 items-start gap-2">
          ...
          <div className="min-w-0">
            <h2 className="text-xs font-bold" id="custom-fields-title">Campos personalizados</h2>
            <p className="text-[0.625rem] leading-4 text-muted-foreground">27 tipos canónicos para CPT y taxonomías, con validación e integridad.</p>
          </div>
        </div>
      ...
      <div className="grid gap-1 rounded-md border border-border bg-surface p-1.5">
        ...
        {fields.length === 0 ? <p className="rounded-md border border-dashed border-border p-3 text-center text-xs text-muted-foreground">Sin campos personalizados.</p> : (
          <div aria-label="Campos personalizados registrados" className="grid gap-1" role="listbox">
            {fields.map((field) => (
              <button aria-selected={selected?.id === field.id} className={`grid min-h-11 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-md border px-2 text-left focus-visible:ring-2 focus-visible:ring-focus lg:min-h-9 ${selected?.id === field.id ? 'border-primary bg-primary-soft' : 'border-border bg-muted/20 hover:bg-muted'}`} key={field.id} onClick={() => selectField(field)} role="option" type="button">
                <span className="min-w-0"><strong className="block truncate text-xs">{field.label}</strong><span className="block truncate font-mono text-[0.625rem] text-muted-foreground">{field.key} · {typeLabels[field.type]}</span></span>
                <span className="max-w-24 truncate text-[0.625rem] font-semibold text-muted-foreground">{ownerLabel(field, contentTypeNames, taxonomyNames)}</span>
              </button>
            ))}
          </div>
        )}
      ...
      <form className="grid gap-2 rounded-md border border-border bg-surface p-2" onSubmit={(event) => { event.preventDefault(); void saveField() }}>
        ...
        <fieldset className="grid grid-cols-2 gap-1.5 rounded-md border border-border p-1.5" disabled={Boolean(selected)}>
          <legend className="px-1 text-[0.625rem] font-bold uppercase tracking-[0.08em] text-muted-foreground">Propietario</legend>
          <label className="grid gap-1 text-xs font-semibold">Tipo de propietario<select className={inputClass} onChange={(event) => setOwnerKind(event.target.value as OwnerKind)} value={draft.ownerKind}><option disabled={contentTypes.length === 0} value="content-type">CPT</option><option disabled={taxonomies.length === 0} value="taxonomy">Taxonomía</option></select></label>
          <label className="grid gap-1 text-xs font-semibold">Entidad<select className={inputClass} onChange={(event) => patchDraft({ ownerId: event.target.value, childFieldIds: [], conditionsJson: '[]', relationId: '', taxonomyId: '' })} value={draft.ownerId}>{draft.ownerKind === 'content-type' ? contentTypes.map((item) => <option key={item.id} value={item.id}>{item.pluralName}</option>) : taxonomies.map((item) => <option key={item.id} value={item.id}>{item.pluralName}</option>)}</select></label>
        </fieldset>

        <div className="grid grid-cols-2 gap-1.5">
          <label className="grid gap-1 text-xs font-semibold">Etiqueta<input className={inputClass} maxLength={160} onChange={(event) => patchDraft({ label: event.target.value })} value={draft.label} /></label>
          <label className="grid gap-1 text-xs font-semibold">Clave<input autoCapitalize="none" className={`${inputClass} font-mono`} maxLength={160} onChange={(event) => patchDraft({ key: event.target.value })} value={draft.key} /></label>
        </div>
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_6rem] gap-1.5">
          <label className="grid gap-1 text-xs font-semibold">Grupo / pestaña<input className={inputClass} maxLength={160} onChange={(event) => patchDraft({ group: event.target.value })} value={draft.group} /></label>
          <label className="grid gap-1 text-xs font-semibold">Orden<input className={inputClass} min="0" onChange={(event) => patchDraft({ order: event.target.value })} type="number" value={draft.order} /></label>
        </div>
        <label className="flex min-h-11 items-center justify-between gap-2 rounded-md border border-border bg-muted/20 px-2 text-xs font-semibold lg:min-h-9"><span><strong className="block">Requerido</strong><span className="font-normal text-muted-foreground">Valida presencia cuando existan registros.</span></span><input checked={draft.required} className="size-4 accent-primary" onChange={(event) => patchDraft({ required: event.target.checked })} type="checkbox" /></label>

        <details className="rounded-md border border-border bg-muted/15 p-1.5">
          <summary className="min-h-9 cursor-pointer select-none py-2 text-xs font-bold">Validación</summary>
          <div className="grid grid-cols-2 gap-1.5 pt-1">
            <label className="grid gap-1 text-[0.625rem] font-semibold">Min length<input className={inputClass} min="0" onChange={(event) => patchDraft({ minLength: event.target.value })} type="number" value={draft.minLength} /></label>
            <label className="grid gap-1 text-[0.625rem] font-semibold">Max length<input className={inputClass} min="0" onChange={(event) => patchDraft({ maxLength: event.target.value })} type="number" value={draft.maxLength} /></label>
            <label className="grid gap-1 text-[0.625rem] font-semibold">Mínimo<input className={inputClass} onChange={(event) => patchDraft({ min: event.target.value })} type="number" value={draft.min} /></label>
            <label className="grid gap-1 text-[0.625rem] font-semibold">Máximo<input className={inputClass} onChange={(event) => patchDraft({ max: event.target.value })} type="number" value={draft.max} /></label>
            <label className="col-span-2 grid gap-1 text-[0.625rem] font-semibold">Pattern<input className={`${inputClass} font-mono`} maxLength={500} onChange={(event) => patchDraft({ pattern: event.target.value })} value={draft.pattern} /></label>
          </div>
        </details>
        ...
        {draft.type === 'relation' ? <label className="grid gap-1 text-xs font-semibold">Relación<select className={inputClass} onChange={(event) => patchDraft({ relationId: event.target.value })} value={draft.relationId}><option value="">Selecciona una relación existente</option>{availableRelations.map((relation) => <option key={relation.id} value={relation.id}>{relation.name}</option>)}</select>{availableRelations.length === 0 ? <span className="font-normal text-muted-foreground">No hay relaciones compatibles. La creación de relaciones pertenece a M09.4.</span> : null}</label> : null}
        ...
        <details className="rounded-md border border-border bg-muted/15 p-1.5">
          <summary className="min-h-9 cursor-pointer select-none py-2 text-xs font-bold">Condiciones y roles</summary>
          <div className="grid gap-2 pt-1">
            <label className="grid gap-1 text-xs font-semibold">Condiciones JSON<span className="font-normal text-muted-foreground">Usa exactamente el contrato canónico de grupos all/any y fieldId.</span><textarea className="min-h-28 resize-y rounded-md border border-border bg-surface p-2 font-mono text-[0.6875rem] font-normal focus-visible:ring-2 focus-visible:ring-focus" onChange={(event) => patchDraft({ conditionsJson: event.target.value })} value={draft.conditionsJson} /></label>
            <fieldset className="grid gap-1"><legend className="text-[0.625rem] font-bold uppercase tracking-[0.08em] text-muted-foreground">Visibilidad por rol</legend>{roles.length === 0 ? <p className="text-[0.625rem] text-muted-foreground">Sin roles canónicos disponibles; F12 formalizará su gestión.</p> : roles.map((role) => <label className="flex min-h-11 items-center gap-2 rounded px-1.5 text-xs hover:bg-muted lg:min-h-9" key={role.id}><input checked={draft.allowedRoleIds.includes(role.id)} className="size-4 accent-primary" onChange={() => toggleStringList('allowedRoleIds', role.id)} type="checkbox" />{role.name}</label>)}</fieldset>
          </div>
        </details>
      ...
      <p className="rounded-md border border-dashed border-border p-2 text-[0.625rem] leading-4 text-muted-foreground">Registros, revisiones y creación de relaciones pertenecen a M09.4; bindings dinámicos pertenecen a M09.5. Esta superficie no simula esas funciones.</p>
    </section>
  )
}
