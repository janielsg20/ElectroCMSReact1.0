...
import { canAccessScreen, editableDemoStore, visibleMenuItemIds, type BackendScreenId } from '../../domain'
...
function DemoStoreSummary() {
  const structure = useEditorProjectStructure()
  const store = editableDemoStore(structure)
  const labels = { orders: 'Pedidos', sales: 'Ventas', stock: 'Inventario' } as const
  const values = { orders: '—', sales: '—', stock: String(store.featuredProduct.stock) } as const
  return <section aria-label="Resumen de tienda compartida" className="grid gap-2 rounded-lg border border-border bg-surface p-3 sm:grid-cols-[minmax(0,1fr)_auto]">
    <div><strong className="block text-xs text-foreground">{store.identity.name}</strong><p className="mt-0.5 text-xs text-muted-foreground">Producto destacado: {store.featuredProduct.name} · {store.featuredProduct.price}</p></div>
    <div className="flex flex-wrap gap-1.5">{store.dashboard.metricOrder.filter((metric) => store.dashboard.visibleMetrics.includes(metric)).map((metric) => <span className="min-h-9 rounded-md border border-border bg-muted/20 px-2 py-1 text-xs" key={metric}><strong>{labels[metric]}</strong>: {values[metric]}</span>)}</div>
  </section>
}
