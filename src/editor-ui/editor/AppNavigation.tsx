...
type AppSection = 'editor' | 'documents' | 'content' | 'design'
type NavigationIcon = 'editor' | 'content' | 'database' | 'palette'
interface SectionDefinition { readonly description: string; readonly icon: NavigationIcon; readonly label: string; readonly panelTitle: string; readonly shortLabel: string }
const sections: Record<AppSection, SectionDefinition> = {
  editor: { description: 'Canvas, estructura y widgets del documento actual.', icon: 'editor', label: 'Editor', panelTitle: 'Editor visual', shortLabel: 'Editor' },
  documents: { description: 'Páginas, plantillas y documentos del proyecto.', icon: 'content', label: 'Documentos', panelTitle: 'Documentos', shortLabel: 'Docs' },
  content: { description: 'Tipos, taxonomías, campos, registros y relaciones CMS.', icon: 'database', label: 'Contenido', panelTitle: 'Contenido CMS', shortLabel: 'CMS' },
  design: { description: 'Temas, presets y paquetes visuales del proyecto.', icon: 'palette', label: 'Diseño', panelTitle: 'Diseño', shortLabel: 'Diseño' },
}
...
const groups: readonly { readonly label: string; readonly sections: readonly AppSection[] }[] = [{ label: 'Builder', sections: ['editor', 'documents'] }, { label: 'CMS', sections: ['content'] }, { label: 'Proyecto', sections: ['design'] }]
...
function ModuleWorkspace({ section, close }: { readonly section: Exclude<AppSection, 'editor'>; readonly close: () => void }) {
  ...
  return (
    <section aria-label={`${item.panelTitle} · módulo principal`} className="fixed bottom-6 right-0 top-10 z-20 hidden min-h-0 flex-col bg-canvas md:flex" data-primary-module={section} role="region" style={{ left: 'var(--rail-width)' }}>
      <header className="flex min-h-12 shrink-0 items-center gap-2 border-b border-border bg-surface px-3">
        ...
        <div className="min-w-0 flex-1"><p className="text-[0.5625rem] font-bold uppercase tracking-[0.1em] text-muted-foreground">{section === 'content' ? 'CMS' : section === 'documents' ? 'Builder' : 'Proyecto'}</p><h1 className="truncate text-sm font-bold text-foreground">{item.panelTitle}</h1></div>
      ...
      <div className="min-h-0 flex-1 overflow-hidden">{section === 'documents' ? <div className="h-full overflow-y-auto p-2 lg:p-3"><TemplateManager /></div> : section === 'content' ? <ProjectDataPanel /> : <ProjectDesignPanel />}</div>
    </section>
  )
}
