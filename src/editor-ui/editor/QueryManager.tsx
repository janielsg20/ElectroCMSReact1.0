...
function makePredicate(source: QuerySource, cms: CmsBackend, contentTypeId: Query['contentTypeId']): QueryPredicate {
  ...
  const users = Object.values(cms.users)
  ...
  if (source === 'author') return { fieldId: null, operator: 'equals', relationId: null, source, taxonomyId: null, value: users[0]?.id ?? '' }
  ...
  if (source === 'repeater') {
    ...
    return {
      fieldId: repeater?.id ?? null,
      operator: 'contains',
      relationId: null,
      source,
      taxonomyId: null,
      value: childId ? { path: [childId], value: '' } : '',
    }
  }
...
function scalarValue(value: JsonValue): string {
...
function updateStructuredOperand(current: JsonValue, value: JsonValue): JsonValue {
  if (current && typeof current === 'object' && !Array.isArray(current) && 'value' in current) return { ...current, value }
  return value
}
...
function ValueEditor({ cms, predicate, onChange, contentTypeId }: {
  ...
  if (predicate.source === 'status') {
    return <select aria-label="Valor de estado" className="min-h-9 w-full rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => onChange({ ...predicate, value: listOperator ? [event.target.value] : event.target.value })} value={Array.isArray(rawOperand) ? String(rawOperand[0] ?? '') : scalarValue(rawOperand)}>{contentStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select>
  }

  if (predicate.source === 'author') {
    const users = Object.values(cms.users).sort((a, b) => lexicalCompare(a.displayName, b.displayName))
    return <select aria-label="Valor de autor" className="min-h-9 w-full rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => onChange({ ...predicate, value: listOperator ? [event.target.value] : event.target.value })} value={Array.isArray(rawOperand) ? String(rawOperand[0] ?? '') : scalarValue(rawOperand)}><option value="">Sin autor</option>{users.map((user) => <option key={user.id} value={user.id}>{user.displayName}</option>)}</select>
  }

  if (predicate.source === 'taxonomy' && predicate.taxonomyId) {
    const terms = Object.values(cms.taxonomyTerms).filter((term) => term.taxonomyId === predicate.taxonomyId).sort((a, b) => lexicalCompare(a.name, b.name))
    return <select aria-label="Valor de término" className="min-h-9 w-full rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => onChange({ ...predicate, value: listOperator ? [event.target.value] : event.target.value })} value={Array.isArray(rawOperand) ? String(rawOperand[0] ?? '') : scalarValue(rawOperand)}><option value="">Selecciona término</option>{terms.map((term) => <option key={term.id} value={term.id}>{term.name}</option>)}</select>
  }
  ...
  if (predicate.source === 'relation' && predicate.relationId) {
    ...
    return <select aria-label="Valor de registro relacionado" className="min-h-9 w-full rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => onChange({ ...predicate, value: listOperator ? [event.target.value] : event.target.value })} value={Array.isArray(rawOperand) ? String(rawOperand[0] ?? '') : scalarValue(rawOperand)}><option value="">Selecciona registro</option>{records.map((record) => <option key={record.id} value={record.id}>{queryRecordLabel(record, cms)}</option>)}</select>
  }

  if (predicate.source === 'date') {
    const current = predicate.value && typeof predicate.value === 'object' && !Array.isArray(predicate.value) ? predicate.value : { field: 'createdAt', value: '' }
    const currentValue = 'value' in current ? current.value : ''
    const inputValue = Array.isArray(currentValue) ? currentValue.join(', ') : scalarValue(currentValue)
    return (
      <div className="grid grid-cols-[7rem_minmax(0,1fr)] gap-1">
        <select aria-label="Campo de fecha" className="min-h-9 rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => onChange({ ...predicate, value: { field: event.target.value, value: currentValue } })} value={'field' in current ? String(current.field) : 'createdAt'}><option value="createdAt">Creado</option><option value="updatedAt">Actualizado</option></select>
        <input aria-label="Valor de fecha" className="min-h-9 min-w-0 rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => onChange({ ...predicate, value: { ...current, value: listOperator ? event.target.value.split(',').map((item) => item.trim()) : event.target.value } })} placeholder={listOperator ? 'inicio, fin' : '2026-08-12T00:00:00.000Z'} value={inputValue} />
      </div>
    )
  }

  const inputValue = Array.isArray(rawOperand) ? rawOperand.map(String).join(', ') : scalarValue(rawOperand)
  return <input aria-label="Valor del predicado" className="min-h-9 w-full rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => {
    ...
    onChange({ ...predicate, value: updateStructuredOperand(predicate.value, next) })
  }} placeholder={listOperator ? 'valor 1, valor 2' : 'Valor'} value={inputValue} />
}
...
function PredicateEditor({ cms, predicate, contentTypeId, onChange, onRemove }: {
  ...
  return (
    <div className="grid gap-1 rounded-md border border-border bg-surface p-1.5">
      <div className="grid grid-cols-[minmax(6rem,0.8fr)_minmax(7rem,1fr)_2rem] gap-1">
      ...
      {predicate.source === 'relation' ? <select aria-label="Relación del predicado" className="min-h-9 w-full rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => {
        const relation = relations.find((candidate) => candidate.id === event.target.value)
        onChange({ ...predicate, relationId: relation?.id ?? null, value: '' })
      }} value={predicate.relationId ?? ''}><option value="">Selecciona relación</option>{relations.map((relation) => <option key={relation.id} value={relation.id}>{relation.name}</option>)}</select> : null}
...
function QueryEditor({ cms, initial, persisted, onSaved, onDeleted }: {
  ...
  const fields = (cms.contentTypes[draft.contentTypeId]?.fieldIds ?? []).flatMap((id) => cms.fields[id] ? [cms.fields[id]] : [])
  ...
  function addSort() {
    const field = fields.find((candidate) => !['checkbox', 'gallery', 'map', 'relation', 'taxonomy', 'repeater', 'group'].includes(candidate.type))
  ...
  async function save() {
    ...
    const result = persisted
      ? await session.updateSavedQuery(draft.id, {
        contentTypeId: draft.contentTypeId, groups: draft.groups, limit: draft.limit, name: draft.name,
        offset: draft.offset, pageSize: draft.pageSize, sorts: draft.sorts,
      })
    ...
    <div className="grid min-h-0 flex-1 gap-2 overflow-y-auto p-2 xl:grid-cols-[minmax(26rem,1fr)_minmax(18rem,0.72fr)] xl:overflow-hidden">
      <section aria-labelledby="query-editor-title" className="min-h-0 rounded-lg border border-border bg-surface xl:overflow-y-auto">
        <header className="sticky top-0 z-10 flex min-h-11 items-center gap-2 border-b border-border bg-surface/95 px-2 backdrop-blur">
          ...
          <button className="min-h-9 rounded-md bg-primary px-3 text-xs font-bold text-on-primary disabled:opacity-50" disabled={!execution.ok || saving} onClick={() => void save()} type="button">{saving ? 'Guardando…' : 'Guardar'}</button>
        </header>

        <div className="grid gap-2 p-2">
          <div className="grid gap-1 sm:grid-cols-2">
            <label className="grid gap-1 text-[0.625rem] font-semibold text-muted-foreground">Nombre<input className="min-h-9 rounded-md border border-border bg-surface px-2 text-xs text-foreground" onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} value={draft.name} /></label>
            <label className="grid gap-1 text-[0.625rem] font-semibold text-muted-foreground">Tipo de contenido<select className="min-h-9 rounded-md border border-border bg-surface px-2 text-xs text-foreground" onChange={(event) => {
          ...
          <section aria-labelledby="query-groups-title" className="grid gap-1.5">
            <div className="flex min-h-9 items-center justify-between"><h3 className="text-xs font-bold" id="query-groups-title">Condiciones</h3><button className="min-h-9 rounded-md border border-border px-2 text-xs font-semibold hover:bg-muted" onClick={addGroup} type="button"><span className="inline-flex items-center gap-1"><Icon name="plus" size={12} />Grupo</span></button></div>
            ...
            {draft.groups.map((group, groupIndex) => (
              <fieldset className="grid gap-1.5 rounded-lg border border-border bg-muted/25 p-1.5" key={groupIndex}>
                ...
                <div className="flex min-h-9 items-center gap-1">
                  ...
                  <select aria-label={`Operador del grupo ${groupIndex + 1}`} className="ml-auto min-h-9 rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => updateGroup(groupIndex, { ...group, operator: event.target.value as QueryGroup['operator'] })} value={group.operator}><option value="all">Todas (AND)</option><option value="any">Alguna (OR)</option></select>
                  <button aria-label={`Añadir predicado al grupo ${groupIndex + 1}`} className="grid size-9 place-items-center rounded-md border border-border bg-surface hover:bg-muted" onClick={() => updateGroup(groupIndex, { ...group, predicates: [...group.predicates, makePredicate('status', cms, draft.contentTypeId)] })} type="button"><Icon name="plus" size={12} /></button>
                  <button aria-label={`Eliminar grupo ${groupIndex + 1}`} className="grid size-9 place-items-center rounded-md text-muted-foreground hover:bg-danger-soft hover:text-danger" onClick={() => setDraft((current) => ({ ...current, groups: current.groups.filter((_, candidate) => candidate !== groupIndex) }))} type="button"><Icon name="close" size={12} /></button>
                </div>
          ...
          <section aria-labelledby="query-sort-title" className="grid gap-1.5 border-t border-border pt-2">
            <div className="flex min-h-9 items-center justify-between"><h3 className="text-xs font-bold" id="query-sort-title">Orden</h3><button className="min-h-9 rounded-md border border-border px-2 text-xs font-semibold hover:bg-muted" onClick={addSort} type="button"><span className="inline-flex items-center gap-1"><Icon name="plus" size={12} />Orden</span></button></div>
            {draft.sorts.map((sort, index) => (
              <div className="grid grid-cols-[minmax(0,1fr)_6rem_2rem] gap-1" key={index}>
                <select aria-label={`Campo de orden ${index + 1}`} className="min-h-9 min-w-0 rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => {
                ...
                }} value={sort.fieldId ?? sort.systemField ?? 'updatedAt'}><optgroup label="Sistema"><option value="createdAt">Creado</option><option value="updatedAt">Actualizado</option><option value="status">Estado</option><option value="id">ID</option></optgroup><optgroup label="Campos">{fields.filter((field) => !['checkbox', 'gallery', 'map', 'relation', 'taxonomy', 'repeater', 'group'].includes(field.type)).map((field) => <option key={field.id} value={field.id}>{field.label}</option>)}</optgroup></select>
                <select aria-label={`Dirección de orden ${index + 1}`} className="min-h-9 rounded-md border border-border bg-surface px-2 text-xs" onChange={(event) => setDraft((current) => ({ ...current, sorts: current.sorts.map((candidate, candidateIndex) => candidateIndex === index ? { ...candidate, direction: event.target.value as QuerySort['direction'] } : candidate) }))} value={sort.direction}><option value="asc">Asc.</option><option value="desc">Desc.</option></select>
                <button aria-label={`Eliminar orden ${index + 1}`} className="grid size-9 place-items-center rounded-md text-muted-foreground hover:bg-danger-soft hover:text-danger" onClick={() => setDraft((current) => ({ ...current, sorts: current.sorts.filter((_, candidate) => candidate !== index) }))} type="button"><Icon name="close" size={12} /></button>
              </div>
            ))}
          </section>

          <div className="grid grid-cols-3 gap-1 border-t border-border pt-2">
            {(['limit', 'offset', 'pageSize'] as const).map((key) => <label className="grid gap-1 text-[0.625rem] font-semibold text-muted-foreground" key={key}>{key === 'pageSize' ? 'Página' : key === 'limit' ? 'Límite' : 'Offset'}<input className="min-h-9 min-w-0 rounded-md border border-border bg-surface px-2 text-xs text-foreground" min={key === 'offset' ? 0 : 1} onChange={(event) => setDraft((current) => ({ ...current, [key]: Number(event.target.value) }))} type="number" value={draft[key]} /></label>)}
          </div>

          {persisted ? <div className="flex items-center justify-end border-t border-border pt-2"><button className={`min-h-9 rounded-md px-3 text-xs font-semibold ${deleteArmed ? 'bg-danger text-white' : 'border border-danger/30 text-danger hover:bg-danger-soft'}`} onClick={() => void remove()} type="button">{deleteArmed ? 'Confirmar eliminar' : 'Eliminar consulta'}</button></div> : null}
