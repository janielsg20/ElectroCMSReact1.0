...
export function LibraryPanel({ activeTab, onTabChange, className = '' }: LibraryPanelProps) {
  ...
  return (
    <aside aria-label="Biblioteca y capas" className={`min-h-0 border-r border-border bg-surface ${className}`}>
      <div className="grid grid-cols-2 border-b border-border bg-muted/60 p-2" role="tablist" aria-label="Panel izquierdo">
        <button aria-selected={activeTab === 'layers'} className={`min-h-11 cursor-pointer rounded-md px-2 text-xs font-bold transition-colors ${activeTab === 'layers' ? 'bg-surface text-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted'}`} onClick={() => onTabChange('layers')} role="tab" type="button">Páginas</button>
      ...
      {activeTab === 'widgets' ? (
        ...
        <div className="h-full overflow-y-auto" role="tabpanel">
          <section className="border-b border-border p-2" aria-labelledby="pages-title">
            <div className="flex min-h-11 items-center justify-between px-1"><h2 className="text-sm font-bold" id="pages-title">Páginas <span className="font-normal text-muted-foreground">· 4</span></h2><button aria-label="Añadir página" className="grid size-11 cursor-pointer place-items-center rounded-md text-primary hover:bg-primary-soft" type="button"><Icon name="plus" size={18} /></button></div>
          ...
          <section className="p-2" aria-labelledby="layers-title">
            <div className="flex min-h-11 items-center justify-between px-1"><h2 className="text-sm font-bold" id="layers-title">Árbol de widgets</h2><button aria-label="Opciones de capas" className="grid size-11 cursor-pointer place-items-center rounded-md hover:bg-muted" type="button"><Icon name="more" size={18} /></button></div>
            <ul aria-label="Árbol de capas" className="mt-1" role="tree">
              {layerItems.map((layer) => (
                <li aria-level={layer.depth + 1} key={layer.id} role="treeitem">
                  <button className={`flex min-h-11 w-full cursor-pointer items-center gap-2 rounded-md pr-2 text-left text-xs transition-colors hover:bg-muted ${layer.id === 'hero-content' ? 'bg-primary-soft font-semibold text-primary-strong' : 'text-muted-foreground'}`} style={{ paddingLeft: `${6 + layer.depth * 12}px` }} type="button">
