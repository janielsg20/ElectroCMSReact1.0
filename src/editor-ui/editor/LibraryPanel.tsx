...
export type LibraryTab = 'widgets' | 'layers' | 'templates' | 'themes'
...
export function LibraryPanel({ activeTab, onTabChange, className = '' }: LibraryPanelProps) {
  ...
  return (
    <aside aria-label="Biblioteca y capas" className={`library-panel flex min-h-0 flex-col border-r border-border bg-surface ${className}`}>
      <div className="grid shrink-0 grid-cols-4 border-b border-border bg-muted/60 p-1.5 lg:p-1" role="tablist" aria-label="Panel izquierdo">
        ...
        <button aria-controls="library-panel-templates" aria-label="Documentos" aria-selected={activeTab === 'templates'} className={`flex min-h-11 cursor-pointer items-center justify-center gap-1 rounded px-1 text-xs font-bold text-primary transition-colors active:bg-primary/15 lg:min-h-9 ${activeTab === 'templates' ? 'bg-primary-soft shadow-sm' : 'hover:bg-primary-soft'}`} id="library-tab-templates" onClick={() => onTabChange('templates')} role="tab" type="button"><Icon name="content" size={14} /><span className="library-tab-label"><span className="library-tab-label--full">Documentos</span><span aria-hidden="true" className="library-tab-label--compact">Docs</span></span></button>
        <button aria-controls="library-panel-themes" aria-selected={activeTab === 'themes'} className={`flex min-h-11 cursor-pointer items-center justify-center gap-1 rounded px-1 text-xs font-bold text-primary transition-colors active:bg-primary/15 lg:min-h-9 ${activeTab === 'themes' ? 'bg-primary-soft shadow-sm' : 'hover:bg-primary-soft'}`} id="library-tab-themes" onClick={() => onTabChange('themes')} role="tab" type="button"><Icon name="palette" size={14} />Diseño</button>
      </div>
