...
export type LibraryTab = 'widgets' | 'layers' | 'templates'
  ...
  return (
    <aside aria-label="Biblioteca y capas" className={`library-panel flex min-h-0 flex-col border-r border-border bg-surface ${className}`} data-electrocms-surface="library-panel">
      <div className="grid shrink-0 grid-cols-3 border-b border-border bg-muted/60 p-1.5 lg:p-1" role="tablist" aria-label="Panel izquierdo">
        ...
        <button aria-controls="library-panel-templates" aria-selected={activeTab === 'templates'} className={`flex min-h-11 cursor-pointer items-center justify-center gap-1 rounded px-1 text-xs font-bold text-primary transition-colors active:bg-primary/15 lg:min-h-9 ${activeTab === 'templates' ? 'bg-primary-soft shadow-sm' : 'hover:bg-primary-soft'}`} id="library-tab-templates" onClick={() => onTabChange('templates')} role="tab" type="button"><Icon name="content" size={14} />Plantillas</button>
      </div>
