...
export function LibraryPanel({ activeTab, onTabChange, className = '' }: LibraryPanelProps) {
  ...
  return (
    <aside aria-label="Biblioteca y capas" className={`min-h-0 border-r border-border bg-surface ${className}`}>
      <div className="grid grid-cols-2 gap-1 border-b border-border bg-canvas/60 p-2" role="tablist" aria-label="Panel izquierdo">
        <button aria-selected={activeTab === 'layers'} className={`min-h-11 cursor-pointer rounded-md px-2 text-xs font-bold transition-colors ${activeTab === 'layers' ? 'bg-surface text-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted'}`} onClick={() => onTabChange('layers')} role="tab" type="button">Páginas y capas</button>
        <button aria-selected={activeTab === 'widgets'} className={`min-h-11 cursor-pointer rounded-md px-2 text-xs font-bold transition-colors ${activeTab === 'widgets' ? 'bg-surface text-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted'}`} onClick={() => onTabChange('widgets')} role="tab" type="button">Componentes</button>
      </div>

      {activeTab === 'widgets' ? (
        <div className="h-full overflow-y-auto p-3" role="tabpanel">
          <div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-bold">Componentes</p><p className="text-[0.6875rem] text-muted-foreground">Selecciona para insertar</p></div><button aria-label="Crear componente" className="grid size-11 cursor-pointer place-items-center rounded-lg text-primary hover:bg-primary-soft" type="button"><Icon name="plus" size={18} /></button></div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            {filteredWidgets.map((widget) => (
              <button className="group min-h-20 cursor-pointer rounded-lg border border-border bg-canvas p-2.5 text-left transition-[border-color,background-color,box-shadow] hover:border-primary hover:bg-primary-soft focus-visible:ring-2 focus-visible:ring-focus" key={widget.label} type="button">
        ...
        <div className="h-full overflow-y-auto" role="tabpanel">
          <section className="border-b border-border p-2.5" aria-labelledby="pages-title">
            <div className="flex min-h-11 items-center justify-between px-1"><div><h2 className="text-sm font-bold" id="pages-title">Páginas</h2><p className="text-[0.625rem] text-muted-foreground">4 páginas del proyecto</p></div><button aria-label="Añadir página" className="grid size-11 cursor-pointer place-items-center rounded-lg text-primary hover:bg-primary-soft" type="button"><Icon name="plus" size={18} /></button></div>
            <ul className="mt-1 grid gap-0.5">
              {pages.map((page) => <li key={page}><button className={`flex min-h-11 w-full cursor-pointer items-center gap-2 rounded-lg px-2 text-left text-xs transition-colors hover:bg-muted ${page === 'Inicio' ? 'bg-primary-soft font-bold text-primary-strong' : 'text-muted-foreground'}`} type="button"><Icon name="editor" size={16} /><span className="flex-1">{page}</span><Icon name="more" size={16} /></button></li>)}
            </ul>
          </section>
          <section className="p-2.5" aria-labelledby="layers-title">
            <div className="flex min-h-11 items-center justify-between px-1"><div><h2 className="text-sm font-bold" id="layers-title">Árbol de widgets</h2><p className="text-[0.625rem] text-muted-foreground">Inicio</p></div><button aria-label="Opciones de capas" className="grid size-11 cursor-pointer place-items-center rounded-lg hover:bg-muted" type="button"><Icon name="more" size={18} /></button></div>
            <ul aria-label="Árbol de capas" className="mt-1" role="tree">
              {layerItems.map((layer) => (
                <li aria-level={layer.depth + 1} key={layer.id} role="treeitem">
                  <button className={`flex min-h-11 w-full cursor-pointer items-center gap-2 rounded-lg pr-2 text-left text-xs transition-colors hover:bg-muted ${layer.id === 'hero-content' ? 'bg-primary-soft font-semibold text-primary-strong' : 'text-muted-foreground'}`} style={{ paddingLeft: `${8 + layer.depth * 14}px` }} type="button">
