...
export function WidgetMediaPlayer({ kind, label = '', source, showControls = true }: WidgetMediaPlayerProps) {
  ...
  return (
    <div aria-label={label || (kind === 'video' ? 'Reproductor de video' : 'Reproductor de audio')} className="overflow-hidden rounded-lg border border-border bg-surface shadow-sm" data-electrocms-widget-control="media-player" role="group">
      {kind === 'video' ? (
        <video
          {...mediaEvents}
